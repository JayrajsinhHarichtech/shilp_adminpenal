...
export default function CommercialForm({ selected, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage(selected ? "Commercial updated!" : "Commercial added!");
        setIsError(false);
        onSaved();
        if (!selected) {
        ...
        setMessage(data.error || "Failed to save Commercial.");
        setIsError(true);
      }
      ...
      console.error(err);
      setMessage("Error saving Commercial.");
      setIsError(true);
      setTimeout(() => setMessage(""), 5000);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="bg-white shadow-md rounded-xl p-6 space-y-6">
      <h2 className="text-2xl font-bold text-gray-800">
        {selected ? "Edit Commercial Project" : "Add Commercial Project"}
      </h2>
